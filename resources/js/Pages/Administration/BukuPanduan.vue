...
const editingBukuPanduan = ref(null);
const previewFile = ref(null);
const search = ref("");
const perPage = ref(10);

// Computed property untuk filter data
const filteredBukuPanduan = computed(() => {
    if (!props.bukuPanduan?.data) return [];
    if (!search.value) return props.bukuPanduan.data;

    const searchTerm = search.value.toLowerCase();
    return props.bukuPanduan.data.filter(
        (item) =>
...
const openEditModal = (bukuPanduan) => {
    editingBukuPanduan.value = bukuPanduan;
    editForm.title = bukuPanduan.title;
    editForm.description = bukuPanduan.description;
    showEditModal.value = true;
};

const submitCreate = () => {
    form.post(route("buku-panduan.store"), {
        preserveScroll: true,
        onSuccess: () => {
            showCreateModal.value = false;
            toast.success("Buku panduan berhasil ditambahkan");
        },
        onError: () => {
            toast.error("Gagal menambahkan buku panduan");
        },
    });
};

const submitEdit = () => {
    editForm.post(route("buku-panduan.update", editingBukuPanduan.value.id), {
        preserveScroll: true,
        onSuccess: () => {
        ...
        onError: () => {
            toast.error("Gagal memperbarui buku panduan");
        },
    });
};

const deleteBukuPanduan = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route("buku-panduan.destroy", id), {
                preserveScroll: true,
                onSuccess: () => {
...
<template>
    <AuthenticatedLayout title="Buku Panduan Management">
        <Head>
            <title>Buku Panduan Management</title>
        </Head>
                    ...
                    <div class="p-6">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                >
                                    <tr
                                        v-for="item in filteredBukuPanduan"
                                        :key="item.id"
                                    >
                                    ...
                                    <tr v-if="filteredBukuPanduan.length === 0">
                                        <td
                                            colspan="5"
                        ...
                        <div class="mt-4 flex items-center justify-between">
                            ...
                            <div class="flex items-center space-x-2">
                                <button
                                    v-for="link in bukuPanduan.links"
                                    :key="link.label"
