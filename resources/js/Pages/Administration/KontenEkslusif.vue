...
const props = defineProps({
    kontenEkslusif: Object,
});

const showCreateModal = ref(false);
const showEditModal = ref(false);
const showPreviewModal = ref(false);
const editingKontenEkslusif = ref(null);
const previewFile = ref(null);
const search = ref("");
const perPage = ref(10);

// Computed property untuk filter data
const filteredKontenEkslusif = computed(() => {
    if (!props.kontenEkslusif?.data) return [];
    if (!search.value) return props.kontenEkslusif.data;

    const searchTerm = search.value.toLowerCase();
    return props.kontenEkslusif.data.filter(
        (item) =>
...
const openEditModal = (kontenEkslusif) => {
    editingKontenEkslusif.value = kontenEkslusif;
    editForm.title = kontenEkslusif.title;
    editForm.description = kontenEkslusif.description;
    showEditModal.value = true;
};

const submitCreate = () => {
    form.post(route("konten-ekslusif.store"), {
        preserveScroll: true,
        onSuccess: () => {
            showCreateModal.value = false;
            toast.success("Konten ekslusif berhasil ditambahkan");
        },
        onError: () => {
            toast.error("Gagal menambahkan konten ekslusif");
        },
    });
};
...
const submitEdit = () => {
    editForm.post(
        route("konten-ekslusif.update", editingKontenEkslusif.value.id),
        {
            preserveScroll: true,
            onSuccess: () => {
                showEditModal.value = false;
                toast.success("Konten ekslusif berhasil diperbarui");
            },
            onError: () => {
                toast.error("Gagal memperbarui konten ekslusif");
            },
        }
    );
};

const deleteKontenEkslusif = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route("konten-ekslusif.destroy", id), {
                preserveScroll: true,
                onSuccess: () => {
...
<template>
    <AuthenticatedLayout title="Konten Ekslusif Management">
        <Head>
            <title>Konten Ekslusif Management</title>
        </Head>
                    ...
                    <div class="p-6">
                        ...
                        >
                            <h2 class="text-2xl font-semibold text-gray-800">
                                Daftar Konten Ekslusif
                            </h2>
                            <div class="flex items-center space-x-4">
                                <TextInput
                                    v-model="search"
                                    type="search"
                                    placeholder="Cari konten ekslusif..."
                                    class="w-64"
                                />
                                <PrimaryButton @click="openCreateModal">
                                    Tambah Konten Ekslusif
                                </PrimaryButton>
                            </div>
                        </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                >
                                    <tr
                                        v-for="item in filteredKontenEkslusif"
                                        :key="item.id"
                                    >
                                    ...
                                    <tr
                                        v-if="
                                            filteredKontenEkslusif.length === 0
                                        "
                                    >
                                        <td
                                            colspan="5"
                        ...
                        <div class="mt-4 flex items-center justify-between">
                            ...
                            <div class="flex items-center space-x-2">
                                <button
                                    v-for="link in kontenEkslusif.links"
                                    :key="link.label"
